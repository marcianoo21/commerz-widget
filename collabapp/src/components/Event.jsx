...
function Event({ selectedDate, events, addEvent, deleteEvent, editEvent }) {
  ...
  const handleSaveEvent = () => {
    if (!eventDescription || !eventHour || !userEmail || !eventTitle) {
      ...
      const eventDetails = { description: eventDescription, hour: eventHour };
      ...
      if (isEditing) {
        ...
        sendEmail(userEmail, eventHour, eventDescription, eventTitle); // Call the sendEmail function with the userEmail
		console.log('User email:', userEmail, "Selected hour:", eventHour, "Description:", eventDescription, "Title:", eventTitle)
      }
      setEventDescription(''); // Clear after adding or editing
      setEventHour('');
      setUserEmail(''); // Clear user email after sending email
	  setEventTitle('')
      setIsEditing(false);
